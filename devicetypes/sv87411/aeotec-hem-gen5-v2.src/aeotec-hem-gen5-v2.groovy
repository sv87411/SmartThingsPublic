 ...
 */
metadata {
	definition (name: "Aeotec HEM Gen5 v2", namespace: "sv87411", author: "Stuart Vickers") {
	...
	tiles (scale: 1) {
		...
		valueTile("power", "device.power", decoration: "flat", width: 2, height: 2) {
			state "power", label:'${currentValue}\nW', unit: "W", icon:"https://raw.githubusercontent.com/sv87411/SmartThingsPublic/master/devicetypes/sv87411/home2.png", backgroundColors:[
				[value: 0, color: "#44b621"],
				[value: 2000, color: "#f1d801"],
				[value: 4000, color: "#d04e00"],
				[value: 6000, color: "#bc2323"]
		...
		standardTile("refresh", "device.refresh", decoration: "flat", width: 2, height: 2) {
			state "default", label:'Refresh', action:"refresh.refresh", icon: "https://raw.githubusercontent.com/sv87411/SmartThingsPublic/master/devicetypes/sv87411/refresh.png"
		}
		
		main "power"
		details("multiTile","power","energy","voltage","current","reset","refresh")
	}
	...
	preferences {
		input (
			...
			image: "https://aeotec.com/images/products/220/z-wave-home-energy-measure@2x.jpg",
			url: "https://aeotec.freshdesk.com/support/solutions/articles/6000161943-home-energy-meter-gen5-user-guide-",
			element: "href"
		)
		...
		input ( name: "logging", title: "Events logged?", type: "boolean", required: false )
        input ( name: "debug_logging", title: "If events logged, include debug?", type: "boolean", required: false )
	}
}
...
def refresh() {
	logging("${device.displayName} - executing all parameter (W, kWh, V, A) refresh()","info")
	def cmds = []
    ...
    // cmds << zwave.meterV3.meterGet(scale: 8)
	return encapSequence(cmds,1000)
}

def refreshPower() {
	logging("${device.displayName} - executing power only refresh()","info")
	def cmds = []
	cmds << zwave.meterV3.meterGet(scale: 0)
	cmds << zwave.meterV3.meterGet(scale: 2)
	return encapSequence(cmds,1000)
...
def refreshVA() {
	logging("${device.displayName} - executing VA only refresh()","info")
	def cmds = []
	cmds << zwave.meterV3.meterGet(scale: 4)
	cmds << zwave.meterV3.meterGet(scale: 5)
	return encapSequence(cmds,1000)
...
def childRefresh(dni) {
    ...
    cmds << response(encap(zwave.meterV3.meterGet(scale: 1), ep))
    ...
    cmds << response(encap(zwave.meterV3.meterGet(scale: 7), ep))
	cmds << response(encap(zwave.meterV3.meterGet(scale: 8), ep))
...
def zwaveEvent(physicalgraph.zwave.commands.meterv3.MeterReport cmd, ep=null) {
	...
	logging("${device.displayName} - MeterReport received, ep: ${((ep) ? ep:0)} name:${type}, value:${cmd.scaledMeterValue}, unit:${unit}", "info") // SV added 'name:' into logging event for InfluxDB logging
	if (ep == null) {
		sendEvent([name: type, value: cmd.scaledMeterValue, unit: unit, displayed: false]) //SV - can be set to true or false. True seems to work when sending events to new smartthings
		...
		getChild(ep)?.sendEvent([name: type, value: cmd.scaledMeterValue, unit: unit, displayed: false]) //SV - can be set to true or false. True seems to work when sending events to new smartthings
...
def parse(String description) {
	def result = []
	logging("${device.displayName} - Parsing: ${description}", "debug")
	if (description.startsWith("Err 106")) {
		...
		if (cmd) {
			logging("${device.displayName} - Parsed: ${cmd}", "debug")
			zwaveEvent(cmd)
		}
	}
}
...
def zwaveEvent(physicalgraph.zwave.commands.securityv1.SecurityMessageEncapsulation cmd) {
	...
	if (encapsulatedCommand) {
		logging("${device.displayName} - Parsed SecurityMessageEncapsulation into: ${encapsulatedCommand}", "debug")
...
def zwaveEvent(physicalgraph.zwave.commands.crc16encapv1.Crc16Encap cmd) {
	...
	if (encapsulatedCommand) {
		logging("${device.displayName} - Parsed Crc16Encap into: ${encapsulatedCommand}", "debug")
		zwaveEvent(encapsulatedCommand)
	} else {
...
def zwaveEvent(physicalgraph.zwave.commands.multichannelv3.MultiChannelCmdEncap cmd) {
	def encapsulatedCommand = cmd.encapsulatedCommand(cmdVersions())
	if (encapsulatedCommand) {
		logging("${device.displayName} - Parsed MultiChannelCmdEncap ${encapsulatedCommand}", "debug")
...
private logging(text, type) {
	if (settings.logging == "true") {
    if  (type == "info") {
    log."$type" text
	}
    if (type == "debug" && settings.debug_logging == "true") {
    log."$type" text
    }
    }
}
...
private multiEncap(physicalgraph.zwave.Command cmd, Integer ep) {
	logging("${device.displayName} - encapsulating command using Multi Channel Encapsulation, ep: $ep command: $cmd","debug")
...
private encapSequence(cmds, Integer delay=1000) {  // SV changed from 250
...
private parameterMap() {[
	...
	[key: "thresholdHEM", num: 4, size: 2, type: "number", def: 0, min: 0, max: 60000, title: "HEM threshold", 
		descr: "Threshold change in wattage to induce a automatic report (Whole HEM)\n0-60000 W"], 
	[key: "thresholdClamp1", num: 5, size: 2, type: "number", def: 0, min: 0, max: 60000, title: "Clamp 1 threshold", 
		descr: "Threshold change in wattage to induce a automatic report (Clamp 1)\n0-60000 W"], 
	[key: "thresholdClamp2", num: 6, size: 2, type: "number", def: 0, min: 0, max: 60000, title: "Clamp 2 threshold", 
		descr: "Threshold change in wattage to induce a automatic report (Clamp 2)\n0-60000 W"], 
	[key: "thresholdClamp3", num: 7, size: 2, type: "number", def: 0, min: 0, max: 60000, title: "Clamp 3 threshold", 
		descr: "Threshold change in wattage to induce a automatic report (Clamp 3)\n0-60000W"], 
	[key: "percentageHEM", num: 8, size: 1, type: "number", def: 0, min: 0, max: 100, title: "HEM percentage", 
		descr: "Percentage change in wattage to induce a automatic report (Whole HEM)\n0-100%"], 
	[key: "percentageClamp1", num: 9, size: 1, type: "number", def: 0, min: 0, max: 100, title: "Clamp 1 percentage", 
		descr: "Percentage change in wattage to induce a automatic report (Clamp 1)\n0-100%"], 
	[key: "percentageClamp2", num: 10, size: 1, type: "number", def: 0, min: 0, max: 100, title: "Clamp 2 percentage", 
		descr: "Percentage change in wattage to induce a automatic report (Clamp 2)\n0-100%"], 
	[key: "percentageClamp3", num: 11, size: 1, type: "number", def: 0, min: 0, max: 100, title: "Clamp 3 percentage", 
...
private optionMap() {[
	...
	[key: "clamp1KVarh", name: "Report kVah of Clamp 1.", value: 16777216, def:null], //Doesn't work
	[key: "clamp2KVarh", name: "Report kVah of Clamp 2.", value: 33554432, def:null], //Doesn't work
	[key: "clamp3KVarh", name: "Report kVah of Clamp 3.", value: 67108864, def:null], //Doesn't work
